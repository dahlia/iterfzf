...
fzf_windows_bin_path = os.path.join(os.path.dirname(__file__), "iterfzf", "fzf.exe")
urllib_retry = 3
...
def get_current_plat_arch():
    ...
    if not machine and sys.platform in ("win32", "cygwin"):
        ...
        try:
            ...
            raise ValueError("unsupported architecture: " + repr((bits, linkage)))
...
class bundle_fzf(distutils.core.Command):
    ...
    def get_plat_name(self, plat=None, arch=None):
        ...
        if plat == "linux":
            ...
            macs = 10, 11, 12
            return ".".join(
                "macosx_10_{0}_{1}".format(mac, arch) for mac in macs for arch in archs
            )
        elif plat == "windows":
            if arch == "386":
                return "win32"
